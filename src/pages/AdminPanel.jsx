            ...
            <thead>
              <tr style={{ background: '#0a1a35' }}>
                {['Name','Badge','Discord ID','Role','Department','Rank','Status','Actions'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#f59e0b', fontSize: '12px', fontWeight: 700, borderBottom: '1px solid #1e4080' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {officers.map((o, i) => (
                <tr key={o.id} style={{ background: i % 2 === 0 ? '#080f1e' : '#0a1525' }}>
                  ...
                  <td style={{ padding: '7px 10px', color: '#475569', fontSize: '12px' }}>{o.discordId}</td>
            ...
            <tbody>
              {auditLog.map((entry, i) => (
                <tr key={entry.id} style={{ background: i % 2 === 0 ? '#080f1e' : '#0a1525' }}>
                  <td style={{ padding: '7px 10px', color: '#475569', fontSize: '12px', whiteSpace: 'nowrap' }}>{entry.timestamp}</td>
                  <td style={{ padding: '7px 10px', color: '#94a3b8' }}>{entry.user}</td>
                  <td style={{ padding: '7px 10px', color: '#e2e8f0' }}>{entry.action}</td>
                  <td style={{ padding: '7px 10px' }}>
                    <span style={{ background: '#1e3060', color: '#4a9eff', borderRadius: '4px', padding: '2px 7px', fontSize: '11px' }}>{entry.module}</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {tab === 'Settings' && (
        <div style={{ maxWidth: '500px' }}>
          <div style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ color: '#e2a84b', fontSize: '14px', fontWeight: 700, letterSpacing: '1px', marginBottom: '4px' }}>COMMUNITY SETTINGS</div>
            {[['Community Name', communityName, setCommunityName], ['Logo URL', logoUrl, setLogoUrl]].map(([l, v, s]) => (
              <div key={l}>
                ...
                <input value={v} onChange={e => s(e.target.value)} style={{ background: '#060d1a', border: '1px solid #1e4080', borderRadius: '4px', color: '#e2e8f0', padding: '8px 10px', fontSize: '14px', fontFamily: 'Ubuntu Mono, monospace', width: '100%', boxSizing: 'border-box' }} />
              </div>
            ))}
            <div>
              <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>CAD ACCENT COLOR</label>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <span style={{ color: '#94a3b8', fontSize: '14px' }}>{accentColor}</span>
              </div>
            </div>
            <button style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', letterSpacing: '1px', marginTop: '8px' }}>
              SAVE SETTINGS
            </button>
          </div>
        </div>
...
const aBtn = (bg, c) => ({ background: bg, border: `1px solid ${c}`, borderRadius: '3px', color: c, padding: '4px 8px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', fontWeight: 600 });
