      ...
      <div style={{ background: `linear-gradient(135deg, #0d1f3c, ${myDept?.color || '#1e4080'}20)`, border: `1px solid ${myDept?.color || '#1e4080'}`, borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <StatusBadge status={myOfficer.status} style={{ fontSize: '13px', padding: '4px 12px' }} />
            {myOfficer.callId && <div style={{ color: '#f59e0b', fontSize: '12px', marginTop: '6px' }}>On Call: {myOfficer.callId}</div>}
          </div>
        </div>
          ...
          <tbody>
            {myCallHistory.map((c, i) => (
              <tr key={c.id} style={{ background: i % 2 === 0 ? '#080f1e' : '#0a1525' }}>
                ...
                <td style={{ padding: '7px 10px', color: '#475569', fontSize: '11px' }}>{c.timestamp}</td>
              </tr>
            ))}
          ...
          {commendations.map(c => (
            <div key={c.id} style={{ background: '#051a05', border: '1px solid #22c55e', borderRadius: '4px', padding: '14px', marginBottom: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ color: '#22c55e', fontWeight: 700, fontSize: '12px' }}>★ {c.type}</span>
                <span style={{ color: '#475569', fontSize: '11px' }}>{c.date}</span>
              </div>
              <div style={{ color: '#64748b', fontSize: '11px', marginBottom: '4px' }}>From: {c.from}</div>
              <div style={{ color: '#94a3b8', fontSize: '12px' }}>{c.note}</div>
            </div>
          ))}
          {complaints.length === 0 && commendations.length > 0 && (
            <div style={{ color: '#22c55e', fontSize: '12px', marginTop: '12px' }}>✓ No complaints on record.</div>
          )}
        </div>
      )}
    </div>
...
function InfoCard({ title, children }) {
  return (
    <div style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '16px' }}>
      <div style={{ color: '#4a9eff', fontSize: '11px', fontWeight: 700, letterSpacing: '1px', marginBottom: '12px', borderBottom: '1px solid #1e3060', paddingBottom: '6px' }}>{title}</div>
      {children}
    </div>
  );
}
