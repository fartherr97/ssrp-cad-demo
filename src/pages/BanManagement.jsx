  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'Ubuntu Mono, monospace' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ background: '#7f1d1d', border: '1px solid #ef4444', borderRadius: '4px', color: '#ef4444', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', fontWeight: 700 }}>
          + Issue Ban
        </button>
      </div>
        ...
        <thead>
          <tr style={{ background: '#0a1a35' }}>
            {['User','Discord ID','Reason','Issued By','Date','Duration','Status','Actions'].map(h => (
              <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#ef4444', fontSize: '12px', fontWeight: 700, borderBottom: '1px solid #1e4080' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {bannedUsers.map((ban, i) => (
            <tr key={ban.id} style={{ background: i % 2 === 0 ? '#080f1e' : '#0a1525', opacity: ban.status !== 'Active' ? 0.5 : 1 }}>
              ...
              <td style={{ padding: '7px 10px', color: '#60a5fa', fontSize: '12px' }}>{ban.discordId}</td>
              ...
              <td style={{ padding: '7px 10px' }}>
                {ban.status === 'Active' && (
                  <button onClick={() => dispatch({ type: 'UNBAN_USER', payload: ban.id })} style={{ background: '#14532d', border: '1px solid #22c55e', borderRadius: '3px', color: '#22c55e', padding: '4px 10px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', fontWeight: 600 }}>
                    Unban
                  </button>
                )}
              </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <form onSubmit={handleBan} style={{ background: '#0d1f3c', border: '1px solid #ef4444', borderRadius: '8px', padding: '24px', maxWidth: '480px', width: '90%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ color: '#ef4444', fontWeight: 700, fontSize: '16px', fontFamily: 'Ubuntu Mono, monospace' }}>🔨 ISSUE BAN</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>REASON *</label>
                <textarea value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))} required rows={3} style={{ ...base, resize: 'vertical' }} />
              </div>
              <div>
                <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>DURATION</label>
              ...
              <button type="submit" style={{ background: '#7f1d1d', border: '1px solid #ef4444', borderRadius: '4px', color: '#ef4444', padding: '10px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>
                CONFIRM BAN
              </button>
            </div>
          </form>
