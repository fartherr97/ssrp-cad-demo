...
export default function RMS() {
  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'Ubuntu Mono, monospace' }}>
            ...
            <thead>
              <tr style={{ background: '#0a1a35' }}>
                {['Date','Case #','Subject','Charges','Officer','Agency','Disposition','Sentence'].map(h => (
                  <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: '#7a9ab8', fontSize: '12px', fontWeight: 700, borderBottom: '1px solid #1e4080' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {warrants.map((w, i) => (
                <tr key={w.id} style={{ background: i % 2 === 0 ? '#080f1e' : '#0a1525' }}>
                  ...
                  <td style={{ padding: '7px 10px' }}>
                    {w.status === 'ACTIVE' && (
                      <button onClick={() => dispatch({ type: 'SERVE_WARRANT', payload: w.id })} style={{ background: '#14532d', border: '1px solid #22c55e', borderRadius: '3px', color: '#22c55e', padding: '3px 8px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>Mark Served</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {tab === 'Field Contacts' && (
        <div>
          <div style={{ color: '#94a3b8', fontSize: '14px', marginBottom: '12px' }}>Field interview / FI card records</div>
          <div style={{ background: '#080f1e', border: '1px solid #1e3060', borderRadius: '4px', padding: '14px', fontSize: '14px', color: '#94a3b8' }}>
            ...
            <div style={{ color: '#64748b', marginTop: '4px', fontSize: '12px' }}>Subject was approached near parked vehicles at 02:15. Stated she was looking for her cat. No criminal activity observed. No further action.</div>
          </div>
        </div>
      )}
        ...
        <div>
          {customRecordTypes.map(type => (
            <div key={type.id} style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '14px', marginBottom: '12px' }}>
              <div style={{ color: '#4a9eff', fontWeight: 700, marginBottom: '8px', fontSize: '15px' }}>📁 {type.name} ({type.fields.length} fields)</div>
              <div style={{ color: '#64748b', fontSize: '12px' }}>Fields: {type.fields.map(f => f.label).join(' | ')}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {showAddArrest && (
        <Modal title="ADD ARREST RECORD" onClose={() => setShowAddArrest(false)}>
          <form onSubmit={submitArrest} style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
            ...
            <button type="submit" style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>
      ...
      {showAddWarrant && (
        <Modal title="ISSUE WARRANT" onClose={() => setShowAddWarrant(false)}>
          <form onSubmit={submitWarrant} style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
            <FormField label="SUBJECT">
            ...
            <button type="submit" style={{ background: '#7f1d1d', border: '1px solid #ef4444', borderRadius: '4px', color: '#ef4444', padding: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>
              ISSUE WARRANT
            </button>
          </form>
        </Modal>
      ...
      {showAddTow && (
        <Modal title="LOG TOW" onClose={() => setShowAddTow(false)}>
          <form onSubmit={submitTow} style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '14px' }}>
...
function FormField({ label, children }) {
  return (
    <div>
      <label style={{ color: '#7a9ab8', fontSize: '12px', letterSpacing: '1px', display: 'block', marginBottom: '5px' }}>{label}</label>
      {children}
    </div>
  );
}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '8px', padding: '24px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflowY: 'auto', fontFamily: 'Ubuntu Mono, monospace' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <span style={{ color: '#4a9eff', fontWeight: 700, fontSize: '15px', letterSpacing: '1px' }}>{title}</span>
