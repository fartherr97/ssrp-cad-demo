  ...
  return (
    <div style={{ padding: '16px', fontFamily: 'Courier New, monospace', display: 'grid', gridTemplateColumns: '1fr 280px', gap: '16px', maxWidth: '1100px' }}>
      <div>
        ...
        {result === null && (
          <div style={{ color: '#334155', fontSize: '13px', textAlign: 'center', padding: '40px' }}>
      ...
      {/* History panel */}
      <div>
        <div style={{ background: '#0a1a35', border: '1px solid #1e4080', borderRadius: '4px', padding: '10px 12px', marginBottom: '0', color: '#4a9eff', fontSize: '11px', fontWeight: 700, letterSpacing: '1px' }}>
          QUERY HISTORY
        </div>
          ...
          {history.map((h, i) => (
            <div key={i} style={{ padding: '8px 12px', borderBottom: '1px solid #0f1e35', display: 'flex', flexDirection: 'column', gap: '2px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#475569', fontSize: '10px' }}>{h.time}</span>
                <span style={{ color: h.result === 'CLEAR' ? '#22c55e' : '#ef4444', fontSize: '10px', fontWeight: 700 }}>{h.result}</span>
              </div>
              <span style={{ color: '#64748b', fontSize: '11px' }}>{h.query}</span>
            </div>
          ))}
          {history.length === 0 && <div style={{ padding: '16px 12px', color: '#334155', fontSize: '12px' }}>No queries yet.</div>}
        </div>
      </div>
    </div>
  );
