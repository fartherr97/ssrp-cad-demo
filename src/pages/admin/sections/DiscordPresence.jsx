...
export default function DiscordPresence() {
  ...
  const setButton = (i, field, val) => {
    setSaved(false);
    setDraft(d => {
      const buttons = d.buttons ? [...d.buttons] : [{ label: '', url: '' }, { label: '', url: '' }];
  ...
  const buttons = draft.buttons || [{ label: '', url: '' }, { label: '', url: '' }];
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <AdminPanel
        ...
        subtitle="Register your Discord application for Rich Presence."
      >
        <div style={{ display: 'grid', gap: 14 }}>
          <SonField label="Client ID">
            <input
              style={{ ...SON_INPUT, fontFamily: 'var(--font-mono)' }}
              value={draft.clientId || ''}
              onChange={e => set('clientId', e.target.value)}
              placeholder="000000000000000000"
            />
          </SonField>
          <SonField label="Icon Name">
            <input
              style={SON_INPUT}
          ...
          <div style={{ fontSize: 11, color: ADMIN.textMute, lineHeight: 1.5 }}>
            Create your application at discord.com/developers/applications. The Client ID and Icon Name must match exactly.
          </div>
        </div>
      </AdminPanel>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 16, alignItems: 'start' }}>
        ...
        >
          ...
          <div style={{ paddingTop: 18 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: ADMIN.text, marginBottom: 4 }}>Button Links</div>
            <div style={{ fontSize: 12, color: ADMIN.textDim, marginBottom: 14 }}>Up to 2 buttons shown on the presence card.</div>
            ...
            <div style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                {buttons.filter(b => b.label).map((b, i) => (
                  <div key={i} style={{
                    marginTop: i === 0 ? 12 : 4, padding: '5px 10px', borderRadius: 4,
                    border: `1px solid ${ADMIN.borderHi}`, fontSize: 11, color: ADMIN.text,
                    background: ADMIN.panel2, cursor: 'default',
                  }}>{b.label}</div>
                ))}
              </div>
            </div>
