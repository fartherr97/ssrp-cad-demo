      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
        ...
        {tab === 'list' && (
          <button onClick={() => { setTab('form'); setEditCiv(null); setCivForm(BLANK_CIV); }}
            style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', fontWeight: 700 }}>
            + New Character
          </button>
        )}
        {tab !== 'list' && (
          <button onClick={() => { setTab('list'); setEditCiv(null); }}
            style={{ background: 'transparent', border: '1px solid #1e3060', borderRadius: '4px', color: '#64748b', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>
            ← Back to List
          </button>
        )}
      </div>
              ...
              return (
                <div key={civ.id} style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                    <div>
                      <div style={{ color: '#fff', fontWeight: 700, fontSize: '16px' }}>{civ.firstName} {civ.lastName}</div>
                      <div style={{ color: '#94a3b8', fontSize: '12px', marginTop: '2px' }}>{civ.gender} • {civ.ethnicity} • DOB: {civ.dob}</div>
                    </div>
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                      {civ.flags.map(f => <StatusBadge key={f} status={f} style={{ fontSize: '10px', padding: '1px 5px' }} />)}
                    </div>
                  </div>
                  ...
                  {selectedCiv?.id === civ.id && (
                    <div style={{ marginTop: '12px', borderTop: '1px solid #1e3060', paddingTop: '10px', fontSize: '12px', color: '#94a3b8' }}>
                        ...
                        <div style={{ marginTop: '8px' }}>
                          ...
                          {civVehicles.map(v => (
                            <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0' }}>
                              ...
                              <StatusBadge status={v.regStatus} style={{ fontSize: '10px' }} />
                            </div>
                          ))}
                        </div>
                      )}
      ...
      {tab === 'form' && (
        <form onSubmit={handleSaveCiv}>
          <div style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '20px' }}>
            <div style={{ color: '#e2a84b', fontSize: '15px', fontWeight: 700, letterSpacing: '1px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '20px' }}>
              ...
              <div>
                <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>DL CLASS</label>
            ...
            <button type="submit" style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '12px 24px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', letterSpacing: '2px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <form onSubmit={handleSaveVehicle} style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '8px', padding: '24px', maxWidth: '480px', width: '90%' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              ...
              <div>
                <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>REGISTRATION</label>
...
function SectionLabel({ label }) {
  return (
    <div style={{ color: '#4a9eff', fontSize: '11px', letterSpacing: '2px', fontWeight: 700, marginBottom: '8px', borderBottom: '1px solid #1e3060', paddingBottom: '4px' }}>
      {label}
    </div>
  );
}

const sBtn = (bg, color) => ({ background: bg, border: `1px solid ${color}`, borderRadius: '3px', color, padding: '5px 8px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' });
