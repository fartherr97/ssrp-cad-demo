    ...
    <div style={{ padding: '16px', fontFamily: 'Ubuntu Mono, monospace' }}>
      ...
      {isMobile && (
        <div style={{ marginBottom: '14px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '8px' }}>
            <span style={{ color: '#4a9eff', fontSize: '15px', fontWeight: 700 }}>RECORD TYPES</span>
            <button onClick={() => { setShowForm(true); setTpl({ name: '', fields: [] }); }} style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '4px 10px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', marginLeft: 'auto' }}>+ New</button>
          </div>
      ...
      <div style={{ display: 'flex', gap: '16px', minHeight: isMobile ? 'auto' : 'calc(100vh - 80px)' }}>
        ...
        {!isMobile && (
          <div style={{ width: '220px', flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <span style={{ color: '#4a9eff', fontSize: '15px', fontWeight: 700 }}>RECORD TYPES</span>
              <button onClick={() => { setShowForm(true); setTpl({ name: '', fields: [] }); }} style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '4px 10px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>+ New</button>
            </div>
            {customRecordTypes.map(type => (
              <div key={type.id} onClick={() => { setSelected(type); setFillMode(null); }}
                ...
                <div style={{ color: '#e2e8f0', fontSize: '14px', fontWeight: 700 }}>📁 {type.name}</div>
                <div style={{ color: '#475569', fontSize: '12px', marginTop: '3px' }}>{type.fields.length} fields</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {selected && !showForm && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                ...
                <button onClick={() => { setTpl({ ...selected, fields: [...selected.fields] }); setShowForm(true); }} style={{ background: 'transparent', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '4px 12px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>Edit Template</button>
                <button onClick={() => setFillMode(selected)} style={{ background: '#1e4080', border: '1px solid #22c55e', borderRadius: '4px', color: '#22c55e', padding: '4px 12px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' }}>+ New Record</button>
              </div>

              {/* Field definitions */}
              <div style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '6px', padding: '16px', marginBottom: '16px' }}>
                <div style={{ color: '#4a9eff', fontSize: '12px', fontWeight: 700, letterSpacing: '1px', marginBottom: '12px' }}>FORM FIELDS</div>
                <div className="table-scroll">
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                  <thead>
                    <tr style={{ background: '#0a1a35' }}>
                      {['#','Label','Type','Required'].map(h => (
                        <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: '#7a9ab8', fontSize: '12px', fontWeight: 700, borderBottom: '1px solid #1e4080' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {selected.fields.map(field => {
                      ...
                      return (
                        <div key={field.id} style={field.type === 'textarea' ? { gridColumn: '1/-1' } : {}}>
                          <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>{field.label.toUpperCase()}{field.required && <span style={{ color: '#ef4444' }}> *</span>}</label>
                          ...
                          {field.type === 'checkbox' && (
                            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                              ...
                              <span style={{ color: '#94a3b8', fontSize: '14px' }}>Yes</span>
                            </label>
                          )}
                          {field.type === 'dropdown' && (
          ...
          {(!selected && !showForm) && (
            <div style={{ color: '#334155', textAlign: 'center', marginTop: '80px', fontSize: '15px' }}>Select a record type or create a new one.</div>
          )}
        </div>
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <form onSubmit={handleSave} style={{ background: '#0d1f3c', border: '1px solid #1e4080', borderRadius: '8px', padding: '24px', maxWidth: '560px', width: '100%', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ color: '#4a9eff', fontWeight: 700, fontSize: '16px', fontFamily: 'Ubuntu Mono, monospace' }}>{tpl.id ? 'EDIT RECORD TYPE' : 'CREATE RECORD TYPE'}</span>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ color: '#94a3b8', fontSize: '12px', display: 'block', marginBottom: '4px' }}>RECORD TYPE NAME *</label>
              <input value={tpl.name} onChange={e => setTpl(t => ({ ...t, name: e.target.value }))} required style={base} />
            </div>

            <div style={{ color: '#94a3b8', fontSize: '12px', marginBottom: '8px' }}>FIELDS ({tpl.fields.length})</div>
            {tpl.fields.map((f, idx) => (
              <div key={f.id} style={{ background: '#060d1a', border: '1px solid #1e3060', borderRadius: '4px', padding: '8px 10px', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                <span style={{ color: '#64748b' }}>{idx + 1}.</span>
                <span style={{ color: '#e2e8f0', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.label}</span>
                <span style={{ color: '#4a9eff', fontSize: '11px', flexShrink: 0 }}>[{f.type}]</span>
                {f.required && <span style={{ color: '#ef4444', fontSize: '11px' }}>*</span>}
            ...
            <div style={{ background: '#060d1a', border: '1px dashed #1e4080', borderRadius: '4px', padding: '12px', marginBottom: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ color: '#94a3b8', fontSize: '12px' }}>ADD FIELD</div>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                <input value={newField.label} onChange={e => setNewField(f => ({ ...f, label: e.target.value }))} placeholder="Field label" style={{ ...base, flex: '2 1 120px' }} />
                <select value={newField.type} onChange={e => setNewField(f => ({ ...f, type: e.target.value }))} style={{ ...base, flex: '1 1 100px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <button type="button" onClick={addField} style={{ background: '#1e4080', border: '1px solid #4a9eff', borderRadius: '4px', color: '#4a9eff', padding: '5px 12px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace', marginLeft: 'auto' }}>+ Add</button>
              </div>
            </div>
...
const sBtn = () => ({ background: '#0a1525', border: '1px solid #1e3060', borderRadius: '3px', color: '#64748b', padding: '2px 6px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Ubuntu Mono, monospace' });
